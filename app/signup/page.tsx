...
export default function SignupPage() {
  ...
  const handleSignup = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.message || 'Unable to sign up');
    ...
    <main className="min-h-screen bg-background text-text">
      ...
      <section className="mx-auto flex min-h-[calc(100vh-96px)] max-w-3xl flex-col justify-center px-6 py-12">
        <div className="rounded-[2rem] bg-white p-10 shadow-soft">
          ...
          <p className="mt-3 text-text/70">Sign up to save favorites, place orders, and track deliveries.</p>
          <form onSubmit={handleSignup} className="mt-10 space-y-6">
            <div>
              <label className="block text-sm font-semibold text-text/80">Full name</label>
              <input
                value={name}
